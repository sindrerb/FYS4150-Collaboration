...
void Ising2D::initializeRandomStateLattice() {
    ...
    for(int ix = 1; ix<nSpin+1;ix++) {
        for(int iy = 1; iy<nSpin+1;iy++) {
            energy += -0.5*(*pseudoLattice[ix][iy])*((*pseudoLattice[ix+1][iy])+(*pseudoLattice[ix-1][iy])+(*pseudoLattice[ix][iy+1])+(*pseudoLattice[ix][iy-1]));
...
void Ising2D::equilibrium(std::string outputFile, int totalMonteCarloCycles, double temperature) {
    //Write equilibrium grahp to this file
    std::ofstream ofile;
    outputFile.append("eq");
    ofile.open(outputFile);
    ofile<<std::setw(15) << "cycles";
    ofile<<std::setw(15) << "accepted";
    ofile<<std::setw(15) << "a/c";
    ofile<<std::setw(15) << "mean(E)";
    ofile<<std::setw(15) << "mean(E)/c";
    ofile<<std::setw(15) << "mean(abs(M))";
    ofile<<std::setw(15) << "mean(abs(E))/c" << std::endl;
    ofile.close();
...
void Ising2D::writeHistogram(std::string outputFile,double temperature, unsigned long *totalHistogram) {
    outputFile.append("hist");
    std::ofstream ofile;
    ofile.open(outputFile, std::ios_base::app);

...
void Matrix::setZeros(int Nrows, int Ncols) {
    rows = Ncols;
    columns = Nrows;
    M = new real*[Nrows];
...
Matrix Matrix::operator=(const Matrix &copy_from) {//Assign matrix eaqual to another
    this->columns=copy_from.columns;
    this->rows=copy_from.rows;
    this->M = new real*[this->columns];
    for(int i = 0; i<this->columns; i++) {
        this->M[i] = new real[this->rows];
        for(int j = 0; j<this->rows ; j++) {
            this->M[i][j] = copy_from.M[i][j];
        }
    }
}

...
\noindent
The equation $(a_{kk}-a_{ll})cs + a_{kl}(c^2-s^2) = 0$ is solved by rewriting the equation using $\Gamma = \frac{a_{ll} - a_{kk}}{2a_{kl}}$ and $t = \frac{s}{c}= \tan(\theta)$ to get the second order polynomial $t^2 + 2\Gamma t-1 = 0$. This gives a solutions for the trigonometrical expression. $t = -\Gamma\pm \sqrt{1+\Gamma^2}$ , $s = tc$ and $c = \frac{1}{\sqrt{1 + \Gamma^2}}$. To avoid problems where $\Gamma$ gets large and possible loss of numerical precision we rewrite $t = \frac{1}{\Gamma + \sqrt{1+\Gamma^2}}$.
\nl
